...
def stream_and_merge_csv() -> None:
    ...
    met_files = readers(
         bucket_url="{1}", file_glob="{3}"
    ).read_csv()
    # tell dlt to merge on date
   
    # NOTE: we load to met_csv table
    load_info = pipeline.run(met_files.with_name("{4}"))
    print(load_info)
...
def read_parquet_and_jsonl_chunked() -> None:
    ...
    jsonl_reader = readers( bucket_url="{1}", file_glob="{3}").read_jsonl(
        chunksize=10000
    )
